...
var DEFAULT_HTTP_MAX_REDIRECTS = 3;
var DEFAULT_HTTP_MAX_BODY_BYTES = 262144;
var httpConfigSchema = external_exports.object({
  allowHosts: external_exports.array(external_exports.string()).default([]),
  timeoutMs: external_exports.number().int().positive().default(DEFAULT_HTTP_TIMEOUT_MS),
  maxRedirects: external_exports.number().int().min(0).default(DEFAULT_HTTP_MAX_REDIRECTS),
  maxBodyBytes: external_exports.number().int().positive().default(DEFAULT_HTTP_MAX_BODY_BYTES),
  blockPrivateNetworks: external_exports.boolean().optional()
});
var projectVerifyConfigSchema = external_exports.object({
  include: external_exports.array(external_exports.string()).optional(),
  http: httpConfigSchema.partial().optional()
});
var verifyConfigSchema = external_exports.object({
  rootDir: external_exports.string().default(process.cwd()),
  timeoutMs: external_exports.number().int().positive().default(15e3),
  sandbox: external_exports.enum(["docker", "local"]).default("docker"),
  include: external_exports.array(external_exports.string()).default(["README.md", "docs/**/*.md"]),
  http: httpConfigSchema.default({})
});
var PROJECT_CONFIG_FILE = "docs-drift.config.json";
...
function mergeHttpAllowHosts(projectAllowHosts, inputAllowHosts, sandbox) {
  const defaults = projectAllowHosts ?? [];
  const overrides = inputAllowHosts ?? [];
  if (sandbox === "local") {
    return Array.from(/* @__PURE__ */ new Set([...defaults, ...overrides]));
  }
...
async function loadProjectVerifyConfig(rootDir) {
  const configPath = import_node_path.default.join(rootDir, PROJECT_CONFIG_FILE);
  try {
    const rawText = await import_promises.default.readFile(configPath, "utf8");
...
async function loadVerifyConfig(input = {}) {
...
async function discoverMarkdownFiles(options) {
  const matches = await (0, import_fast_glob.default)(options.include, {
    cwd: options.rootDir,
    onlyFiles: true,
    unique: true,
    dot: false
  });
  return matches.sort().map((file) => import_node_path2.default.join(options.rootDir, file));
}
...
var VFile = class {
   ...
   */
  get basename() {
    return typeof this.path === "string" ? import_node_path3.default.basename(this.path) : void 0;
  }
  /**
  ...
  set basename(basename) {
    ...
    this.path = import_node_path3.default.join(this.dirname || "", basename);
  }
  /**
   ...
   */
  get dirname() {
    return typeof this.path === "string" ? import_node_path3.default.dirname(this.path) : void 0;
  }
  /**
  ...
  set dirname(dirname) {
    ...
    this.path = import_node_path3.default.join(dirname || "", this.basename);
  }
  /**
   ...
   */
  get extname() {
    return typeof this.path === "string" ? import_node_path3.default.extname(this.path) : void 0;
  }
  /**
  ...
  set extname(extname) {
    ...
    this.path = import_node_path3.default.join(this.dirname, this.stem + (extname || ""));
  }
  /**
   ...
   */
  get stem() {
    return typeof this.path === "string" ? import_node_path3.default.basename(this.path, this.extname) : void 0;
  }
  /**
  ...
  set stem(stem) {
    ...
    this.path = import_node_path3.default.join(this.dirname || "", stem + (this.extname || ""));
  }
...
function assertPart(part, name) {
  if (part && part.includes(import_node_path3.default.sep)) {
    throw new Error(
      "`" + name + "` cannot be a path: did not expect `" + import_node_path3.default.sep + "`"
    );
  }
}
...
function parseInfo(rawLang, rawMeta) {
...
function buildBlockId(filePath, lineStart, metadata) {
  if (metadata.id) {
    return metadata.id;
  }
...
async function parseMarkdownFile(options) {
  const text3 = await import_promises2.default.readFile(options.filePath, "utf8");
  const ast = processor.parse(text3);
  const blocks = [];
...
async function parseMarkdownFiles(filePaths, allowedLanguages) {
  const all2 = await Promise.all(filePaths.map((filePath) => parseMarkdownFile({ filePath, allowedLanguages })));
  return all2.flatMap((entry) => entry.blocks);
}
...
function buildExecutionPlan(options) {
  ...
  for (const block of options.blocks) {
    ...
    const expectedJsonBlock = expectedJsonRef ? jsonById.get(expectedJsonRef) : void 0;
    let expectedJsonEquals = void 0;
    if (expectedJsonBlock) {
      try {
        expectedJsonEquals = JSON.parse(expectedJsonBlock.content);
      } catch {
        expectedJsonEquals = void 0;
      }
    }
    const expectedExitCode = block.language === "bash" || block.language === "sh" ? parseNumber(getMetadataValue(block.metadata, ["docs-drift:expect-exit", "expectExit"])) ?? 0 : parseNumber(getMetadataValue(block.metadata, ["docs-drift:expect-exit", "expectExit"]));
    plan.push({
      block,
      expected: {
        expectedExitCode,
...
// ../../packages/assertions-http/src/index.ts
function resolveBody(result) {
  return result.http?.response.body ?? result.stdout;
}
var HttpStatusAssertion = class {
  name = "http-status";
  async evaluate(result, expected) {
    ...
    return {
      ...
      message: actualStatus === expected.expectedStatus ? `HTTP status matched (${expected.expectedStatus}).` : `Expected HTTP status ${expected.expectedStatus}, got ${actualStatus}.`
    };
  }
};
var BodyContainsAssertion = class {
  name = "body-contains";
  async evaluate(result, expected) {
...
function buildJsonPath(pathParts) {
...
function sortJsonValue(value) {
  ...
  const entries = Object.entries(value).sort(([a], [b]) => a.localeCompare(b)).map(([key, entryValue]) => [key, sortJsonValue(entryValue)]);
...
function omitJsonPaths(value, ignoredPaths, pathParts = []) {
  const currentPath = buildJsonPath(pathParts);
  if (currentPath && ignoredPaths.has(currentPath)) {
    return OMIT;
  }
  if (Array.isArray(value)) {
    const nextItems = value.map((item, index2) => omitJsonPaths(item, ignoredPaths, [...pathParts, String(index2)])).filter((item) => item !== OMIT);
    return nextItems;
  }
  if (isPlainObject2(value)) {
    const nextEntries = Object.entries(value).map(([key, entryValue]) => [key, omitJsonPaths(entryValue, ignoredPaths, [...pathParts, key])]).filter((entry) => entry[1] !== OMIT);
    return Object.fromEntries(nextEntries);
  }
  return value;
}
function normalizeJson(value, ignorePaths = []) {
...
function collectJsonDifferences(expected, actual, path7 = "$") {
  if (!isPlainObject2(expected) && !Array.isArray(expected) && !isPlainObject2(actual) && !Array.isArray(actual)) {
    if (Object.is(expected, actual)) {
      return [];
    }
    return [`${path7}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`];
  }
  if (Array.isArray(expected) || Array.isArray(actual)) {
    if (!Array.isArray(expected) || !Array.isArray(actual)) {
      return [`${path7}: expected ${Array.isArray(expected) ? "array" : typeof expected}, got ${Array.isArray(actual) ? "array" : typeof actual}`];
    }
    const differences2 = [];
    if (expected.length !== actual.length) {
      differences2.push(`${path7}: expected array length ${expected.length}, got ${actual.length}`);
    }
    for (let index2 = 0; index2 < Math.min(expected.length, actual.length); index2 += 1) {
      differences2.push(...collectJsonDifferences(expected[index2], actual[index2], `${path7}[${index2}]`));
    }
    return differences2;
  }
  if (!isPlainObject2(expected) || !isPlainObject2(actual)) {
    return [`${path7}: expected ${typeof expected}, got ${typeof actual}`];
  }
  const keys = Array.from(/* @__PURE__ */ new Set([...Object.keys(expected), ...Object.keys(actual)])).sort((a, b) => a.localeCompare(b));
  const differences = [];
  for (const key of keys) {
    if (!(key in expected)) {
      differences.push(`${path7}.${key}: unexpected key present`);
      continue;
    }
    if (!(key in actual)) {
      differences.push(`${path7}.${key}: missing key`);
      continue;
    }
    differences.push(...collectJsonDifferences(expected[key], actual[key], `${path7}.${key}`));
  }
  return differences;
}
function compareJson(expected, actual, ignorePaths = []) {
...
// ../../packages/assertions-json/src/index.ts
...
function resolveBody2(result) {
  return result.http?.response.body ?? result.stdout;
}
function hasJsonPath(value, keyPath) {
...
var JsonEqualsAssertion = class {
  ...
  async evaluate(result, expected) {
    ...
    const parsed = safeParseJson(resolveBody2(result).trim());
    if (!parsed.ok) {
      return {
        assertion: this.name,
        ok: false,
...
var JsonHasKeysAssertion = class {
  name = "json-has-keys";
  async evaluate(result, expected) {
...
// ../../packages/assertions-core/src/index.ts
var ExitCodeAssertion = class {
  name = "exit-code";
  async evaluate(result, expected) {
    ...
    const ok2 = result.exitCode === expectedCode;
    return {
      assertion: this.name,
      ok: ok2,
      message: ok2 ? `Exit code matched (${expectedCode}).` : `Expected exit code ${expectedCode}, got ${result.exitCode}.`
    };
  }
};
var StdoutContainsAssertion = class {
  ...
  async evaluate(result, expected) {
    ...
    const ok2 = result.stdout.includes(needle);
    return {
      assertion: this.name,
      ok: ok2,
      message: ok2 ? `stdout contains '${needle}'.` : `Expected stdout to contain '${needle}'.`
    };
  }
};
async function evaluateAssertions(result, expected, assertions) {
  const outcomes = [];
...
async function runExecutionPlan(options) {
  const startedAt = (/* @__PURE__ */ new Date()).toISOString();
  const baseAssertions = [new ExitCodeAssertion(), new StdoutContainsAssertion()];
  ...
  for (const item of options.plan) {
    ...
    const ok2 = execution.ok && assertionResults.every((entry) => entry.ok);
    results.push({
      block: item.block,
      execution,
...
// ../../packages/runner-http/src/http-policy.ts
var import_node_net = require("node:net");
var LOCALHOST_ALLOWLIST = ["localhost", "127.0.0.1", "::1"];
...
function matchesAllowedHost(pattern, hostname) {
...
function parseIpv6Bytes(value) {
  ...
  for (let index2 = 0; index2 < groups.length; index2 += 1) {
    ...
    bytes[index2 * 2] = valueNumber >> 8 & 255;
    bytes[index2 * 2 + 1] = valueNumber & 255;
  }
  return bytes;
}
function getIpLiteralDetails(hostname) {
  const normalized = normalizeHost(hostname);
  const ipVersion = (0, import_node_net.isIP)(normalized);
  if (ipVersion === 4) {
...
function resolveHttpAllowlist(context) {
  ...
  return Array.from(/* @__PURE__ */ new Set([...defaults, ...configuredHosts]));
}
function resolveEffectiveHttpOptions(context) {
  return {
    allowHosts: resolveHttpAllowlist(context),
    timeoutMs: context.http?.timeoutMs ?? 5e3,
    maxRedirects: context.http?.maxRedirects ?? 3,
    maxBodyBytes: context.http?.maxBodyBytes ?? 262144,
    blockPrivateNetworks: context.http?.blockPrivateNetworks ?? context.sandbox !== "local"
  };
}
function buildBlockedHostMessage(url, hostname, activeAllowlist) {
...
function buildPrivateNetworkBlockedMessage(url, hostname, category) {
  return [
...
function evaluateHttpHostPolicy(url, context) {
...
function parseCurlCommand(content3) {
  const tokens = tokenizeCurlCommand(content3);
  if (tokens[0] !== "curl") {
  ...
  for (let index2 = 1; index2 < tokens.length; index2 += 1) {
    ...
    if (token === "-X" || token === "--request") {
      method = nextToken(tokens, index2, token).toUpperCase();
      methodExplicitlySet = true;
      index2 += 1;
      continue;
    }
    if (token === "-H" || token === "--header") {
      const [name, value] = parseHeader(nextToken(tokens, index2, token));
      headers[name] = value;
      index2 += 1;
      continue;
    }
    if (token === "-d" || token === "--data" || token === "--data-raw" || token === "--data-binary") {
      bodyParts.push(nextToken(tokens, index2, token));
    ...
    if (token === "--url") {
      url = nextToken(tokens, index2, token);
      index2 += 1;
      continue;
    }
  ...
  const body = bodyParts.length > 0 ? bodyParts.join("&") : void 0;
...
function buildFailedExecutionResult(request, message, startedAt, status = 0) {
  return {
    blockId: "",
    runnerName: "",
    ok: false,
...
function buildBodyTooLargeMessage(url, maxBodyBytes, observedBytes) {
...
function resolveRedirectTarget(location, currentUrl) {
  return new URL(location, currentUrl);
}
function shouldRewriteMethodForRedirect(status, method) {
  return status === 303 || (status === 301 || status === 302) && method !== "GET" && method !== "HEAD";
}
async function readResponseBody(response, maxBodyBytes, url) {
  ...
  for (let redirectCount = 0; redirectCount <= httpOptions.maxRedirects; redirectCount += 1) {
    ...
    try {
      ...
      if (REDIRECT_STATUS_CODES.has(response.status)) {
        ...
        if (!location) {
          const bodyResult2 = await readResponseBody(response, httpOptions.maxBodyBytes, currentUrl.toString());
          return {
            blockId: "",
            runnerName: "",
            ok: true,
            exitCode: 0,
            stdout: bodyResult2.ok ? bodyResult2.body : "",
            stderr: bodyResult2.ok ? "" : bodyResult2.message,
            durationMs: Date.now() - startedAt,
            http: {
              ...
              response: {
                ...
                body: bodyResult2.ok ? bodyResult2.body : ""
              }
            }
          };
        }
        ...
        currentRequest = {
          ...
          headers: { ...currentRequest.headers },
          body: shouldRewrite ? void 0 : currentRequest.body
        };
        if (shouldRewrite) {
...
// ../../packages/runner-shell/src/index.ts
var import_node_child_process = require("node:child_process");
function runCommand(command, args, input, cwd, timeoutMs) {
  return new Promise((resolve) => {
    const start = Date.now();
    const child = (0, import_node_child_process.spawn)(command, args, { cwd, stdio: "pipe" });
    let stdout = "";
    let stderr = "";
    let timedOut = false;
...
async function executeInLocalShell(content3, context) {
  return runCommand("bash", ["-lc", "cat >/tmp/docs-drift-snippet.sh && bash /tmp/docs-drift-snippet.sh"], content3, context.cwd, context.timeoutMs);
}
async function executeInDocker(content3, context) {
  const args = [
    "run",
    "--rm",
    "-i",
  ...
  return runCommand("docker", args, content3, context.cwd, context.timeoutMs);
}
...
// ../../packages/reporter-markdown/src/index.ts
var import_promises3 = __toESM(require("node:fs/promises"), 1);
var import_node_path5 = __toESM(require("node:path"), 1);

// ../../packages/reporter-core/src/index.ts
...
// ../../packages/reporter-markdown/src/index.ts
function formatFailureStderr(stderr) {
...
function renderMarkdownReport(report) {
  ...
  for (const result of report.results) {
    ...
    const fileWithLine = `${import_node_path5.default.relative(process.cwd(), result.block.filePath)}:${result.block.lineStart}`;
...
var MarkdownReporter = class {
  ...
  async publish(report) {
    const content3 = renderMarkdownReport(report);
    if (this.writeToStdout) {
      process.stdout.write(`${content3}
`);
    }
    if (this.outputFile) {
      await import_promises3.default.mkdir(import_node_path5.default.dirname(this.outputFile), { recursive: true });
      await import_promises3.default.writeFile(this.outputFile, content3, "utf8");
    }
  }
};
...
function runCommand2(command, args) {
  return new Promise((resolve) => {
    const child = (0, import_node_child_process2.spawn)(command, args, { stdio: "ignore" });
    child.on("error", () => resolve(false));
    child.on("close", (code) => resolve(code === 0));
  });
}
async function resolveSandbox(config) {
  if (config.sandbox === "local") {
    return "local";
  }
  const hasDocker = await runCommand2("docker", ["version"]);
  if (!hasDocker) {
...
async function verifyDocs(input = {}) {
  const invocationDir = process.env.INIT_CWD ?? process.cwd();
  const resolvedRoot = input.rootDir ? import_node_path6.default.resolve(invocationDir, input.rootDir) : invocationDir;
  ...
  const summaryPath = input.summaryFile ? import_node_path6.default.resolve(config.rootDir, input.summaryFile) : void 0;
  await new MarkdownReporter({ outputFile: summaryPath, writeToStdout: true }).publish(report);
  return hasFailures(report) ? 1 : 0;
}

// src/index.ts
async function appendStepSummary(summaryPath, env = process.env) {
  const githubStepSummary = env.GITHUB_STEP_SUMMARY;
  if (!githubStepSummary) {
    return;
  }
  const summary = await import_promises4.default.readFile(summaryPath, "utf8");
  await import_promises4.default.appendFile(githubStepSummary, `${summary}
`, "utf8");
}
...
function resolveGitHubActionVerifyInput(env = process.env) {
  ...
  const sandbox = sandboxValue === "local" || sandboxValue === "docker" ? sandboxValue : void 0;
  const readFromEnv = (inputEnvName, fallbackEnvName) => {
    ...
    return typeof fallbackValue === "string" && fallbackValue.trim() !== "" ? fallbackValue : void 0;
  };
  return {
    rootDir,
    summaryFile,
    sandbox,
    timeoutMs: parseNumber2(readFromEnv("INPUT_TIMEOUT_MS", "DOCS_DRIFT_TIMEOUT_MS")),
    httpAllowHosts: parseHostList(readFromEnv("INPUT_HTTP_ALLOW_HOSTS", "DOCS_DRIFT_HTTP_ALLOW_HOSTS")),
    httpTimeoutMs: parseNumber2(readFromEnv("INPUT_HTTP_TIMEOUT_MS", "DOCS_DRIFT_HTTP_TIMEOUT_MS")),
    httpMaxRedirects: parseNumber2(readFromEnv("INPUT_HTTP_MAX_REDIRECTS", "DOCS_DRIFT_HTTP_MAX_REDIRECTS")),
    httpMaxBodyBytes: parseNumber2(readFromEnv("INPUT_HTTP_MAX_BODY_BYTES", "DOCS_DRIFT_HTTP_MAX_BODY_BYTES")),
...
async function runGitHubAction(env = process.env, executeVerifyDocs = verifyDocs) {
  const options = resolveGitHubActionVerifyInput(env);
  const exitCode = await executeVerifyDocs(options);
  await appendStepSummary(import_node_path7.default.join(options.rootDir, options.summaryFile), env);
  return exitCode;
}
